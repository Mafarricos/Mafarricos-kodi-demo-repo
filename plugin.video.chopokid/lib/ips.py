...
def cookie():
    ...
    while numero < 20:
        ...
        payload = {'url':'https://www.gnula2.co/estrenos/','proxyServerId':comienza,'demo':'0','frontOrigin':'https://www.croxyproxy.rocks'}   
        x = requests.post(url,headers=headers,data=payload,verify=False,timeout=3)
        if 'access-control-allow-origin' in x.headers:
